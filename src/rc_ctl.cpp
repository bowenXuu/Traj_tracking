...
void pose_set_callback(const geometry_msgs::PoseStampedConstPtr &pose_set_msg)
{
    ...
    if(fabs(z_take_off-pose_set_msg->pose.position.z) < 0.100)
    {
        take_off = true ;
    }
...
void rc_callback(const mavros_msgs::RCInConstPtr &rc_msg)
{
    ...
    if (rc_msg->channels[4] < 1250)
    {
        ...
        mission_state = INIT;
        cout << "px4_state.mode is " << px4_state.mode << endl;
    }
    ...
    if (mission_state != INIT)
    {
        ...
        if (mission_state == POSITION && take_off == true)
        {
            T_W_B_set.position.x += rc_ch[3] * MAX_MANUAL_VEL * delta_t * (RC_REVERSE_PITCH ? -1 : 1);
            T_W_B_set.position.y += rc_ch[1] * MAX_MANUAL_VEL * delta_t * (RC_REVERSE_ROLL ? 1 : -1);
            T_W_B_set.position.z += rc_ch[2] * MAX_MANUAL_VEL * delta_t * (RC_REVERSE_THROTTLE ? -1 : 1);
        }
